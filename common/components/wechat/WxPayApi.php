...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(Yii::$app->params['wechat']['notify_url']);//异步通知url
		}
		
		$inputObj->SetAppid(Yii::$app->params['wechat']['appId']);//公众账号ID
		$inputObj->SetMch_id(Yii::$app->params['wechat']['mchId']);//商户号
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, Yii::$app->params['wechat']['sslCert_path']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, Yii::$app->params['wechat']['sslKey_path']);
		}
		//post提交方式
	...
	{
		//如果不需要上报数据
		if(Yii::$app->params['wechat']['report_levenl'] == 0){
			return;
		}
		//如果仅失败上报
		if(Yii::$app->params['wechat']['report_levenl'] == 1 &&
	...
	public static function closeOrder($inputObj, $timeOut = 6)
	{
		...
		//检测必填参数
		if(!$inputObj->IsOut_trade_noSet()) {
			throw new Exception("订单查询接口中，out_trade_no至少填一个！");
		}
		$inputObj->SetAppid(Yii::$app->params['wechat']['appId']);//公众账号ID
		$inputObj->SetMch_id(Yii::$app->params['wechat']['mchId']);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
